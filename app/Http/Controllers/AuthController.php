...
class AuthController extends Controller
{
    ...
    public function verifyEditOtp(Request $request)
    {
        ...
        return response()->json(['message' => 'OTP verified , your phone updated successfully'], 201);
    }
    ...
    // support and complaints email (any user can send )
    public function send(Request $request)
    {
        ...
        $name = Auth::user()->name['en'] ?? Auth::user()->name;
        ...
        Mail::raw($emailContent, function ($message) use ($adminEmail) {
            $message->to($adminEmail)
                ->subject('support and complaints');
        });

        $admin = User::where('role_id',5)->first();
        $this->notificationService->notifyUser(
            $admin->id ,
            "support and complaints email",
            "there is new email , check it !!",
        );
        return response()->json([
            'status' => 'success',
